...
/**
  ...
  */
class BgTestCrateTest extends BgTestService with BgTestCrate {

  private val serviceOne = testWith(new BgPersistenceJournalCrate() {

    override def systemName: String = defaultSystemName + portBase

    override def portBase = 1
  })
  private val serviceTwo = testWith(new BgPersistenceJournalCrate() {
  ...
  feature("Testing of two services is possible with crate db") {
