...
object Internals {
  type Wrapper = RoachSqlishValueHolder
  type Ref = Any

  case class RoachSqlishValueHolder(value: Option[Ref])
  ...
  def unSqlish(x: Ref): Option[Ref] = x match {
    case RoachSqlishValueHolder(None) => None
    case RoachSqlishValueHolder(Some(v: Array[_])) => Some(v.map(it => unSqlish(it)).filter(_.isDefined).map(_.get))
    case RoachSqlishValueHolder(Some(v: List[_])) => Some(v.map(it => unSqlish(it)).filter(_.isDefined).map(_.get).asJava)
    case RoachSqlishValueHolder(Some(v: java.util.List[_])) => Some(v.asScala.map(it => unSqlish(it)).filter(_.isDefined).map(_.get).asJava)
    case RoachSqlishValueHolder(Some(v: Map[_, _])) => Some(v
      .map(it => (it._1, unSqlish(it._2)))
      .filter(_._2.isDefined)
      .map(it => (it._1, it._2.get))
      .asJava
    )
    case RoachSqlishValueHolder(Some(v: java.util.Map[_, _])) => Some(v
      .asScala
      .map(it => (it._1, unSqlish(it._2)))
      .filter(_._2.isDefined)
      .map(it => (it._1, it._2.get))
    ...
    case RoachSqlishValueHolder(Some(v)) => unSqlish(v)
    case null => None
    case Some(v) => Some(v)
    case v => Some(v)
  }

  def unSqlish(x: Map[String, RoachSqlishValueHolder]): java.util.Map[String, Ref] = {
  ...
  abstract class IdentitySqlConverter[A]() extends SqlConverter[A] {
    override def toSqlType(a: A): Wrapper = RoachSqlishValueHolder(Option(a))
  ...
  class OptionSqlConverter[A: ClassTag](implicit val conv: SqlConverter[A]) extends SqlConverter[Option[A]] {
    override def toSqlType(a: Option[A]): Wrapper = RoachSqlishValueHolder(a)
  ...
  class ListSqlConverter[A: ClassTag](implicit val conv: SqlConverter[A]) extends SqlConverter[List[A]] {
    ...
    override def toSqlType(a: List[A]): Wrapper = RoachSqlishValueHolder(
  ...
  class ArraySqlConverter[A: ClassTag](implicit val conv: SqlConverter[A]) extends SqlConverter[Array[A]] {
    override def toSqlType(a: Array[A]): Wrapper = RoachSqlishValueHolder(Option(a.map(it => conv.toSqlType(it))))
  ...
  class MapSqlConverter[A](implicit val conv: SqlConverter[A]) extends SqlConverter[Map[String, A]] {
    override def toSqlType(a: Map[String, A]): Wrapper = a match {
      case null => RoachSqlishValueHolder(None)
      case notNull => RoachSqlishValueHolder(Option(notNull.map(it => (it._1, conv.toSqlType(it._2))).asJava))
    }
