...
object Message {
  ...
  def apply[P <: AnyRef](target: String, headers: Map[String, String], payload: Payload[P]): Message[P] = new Message(target, UUID.randomUUID().toString, headers + createTimestamp, payload)

  private def createTimestamp = {
    MessageHeaders.HEADER_CREATION_TIME -> System.currentTimeMillis().toString
  }

  def apply[P <: AnyRef](target: String, payload: Payload[P]): Message[P] = new Message(target, UUID.randomUUID().toString, Map() + createTimestamp, payload)
