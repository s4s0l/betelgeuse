...
trait TimeoutActor extends HackedActor with ActorLogging {
  ...
  val timeoutScheduleDispatcher: ExecutionContextExecutor = context.dispatcher
  ...
  final def timeoutDelay(duration: FiniteDuration = timeoutTime): Unit = {
    cancelTimeoutTimer()
    timeoutTimer = Some(context.system.scheduler.scheduleOnce(timeoutTime, self, timeoutSelfMessage)(timeoutScheduleDispatcher))
  }
