...
/**
  ...
  */
class CrateDbLocks(val schema: String = "locks", locksTable: String = "locks")
  ...
  override def isLocked(lockName: String)(implicit session: DBSession): Boolean = {
  ...
  def getLockingParty(lockName: String)(implicit session: DBSession): Option[(String, String)] = {
    ...
    sql"""
      ...
      .first().apply().filter(_._3.getTime > now.getTime)
      .map(x => (x._1, x._2))
  }
    ...
    if (!exists) {
      dbSession.execute(
        s"""create table $schema.$locksTable(
                ...
                when_locked timestamp not null,
                when_overdue TIMESTAMP not null,
                by_who string,
