...
import org.s4s0l.betelgeuse.akkacommons.clustering.pubsub.BetelgeuseAkkaClusteringPubSubExtension
import org.s4s0l.betelgeuse.akkacommons.utils.PubSubWithDefaultMediator

/**
  * @author Marcin Wielgus
  */
class BetelgeuseAkkaClusteringReceptionistExtension(private val system: ExtendedActorSystem) extends Extension {
  ...
  /**
    ...
    */
  val pubSubMediator: ActorRef = BetelgeuseAkkaClusteringPubSubExtension(system).mediator
  ...
  def registerByName(name: String, ref: ActorRef)(implicit sender: ActorRef = Actor.noSender): Unit =
    BetelgeuseAkkaClusteringPubSubExtension(system).registerByName(name, ref, pubSubMediator)
  ...
  def register(actor: ActorRef)(implicit sender: ActorRef = Actor.noSender): Unit =
    BetelgeuseAkkaClusteringPubSubExtension(system).register(actor, pubSubMediator)
  ...
  def unregister(actor: ActorRef)(implicit sender: ActorRef = Actor.noSender): Unit =
    BetelgeuseAkkaClusteringPubSubExtension(system).unregister(actor.path.toStringWithoutAddress, pubSubMediator)
  ...
  def subscribe(topic: String, actor: ActorRef)(implicit sender: ActorRef = Actor.noSender): Unit =
    BetelgeuseAkkaClusteringPubSubExtension(system).subscribe(topic, actor, pubSubMediator)
  ...
  def unsubscribe(topic: String, actor: ActorRef)(implicit sender: ActorRef = Actor.noSender): Unit =
    BetelgeuseAkkaClusteringPubSubExtension(system).unsubscribe(topic, actor, pubSubMediator)


  /**
    * Publishes to pub sub
    */
  def publish(topicName: String, msg: Any): Unit = BetelgeuseAkkaClusteringPubSubExtension(system).publish(topicName, msg, pubSubMediator)

  /**
    * sends to pub sub directly
    */
  def send(ref: String, msg: Any): Unit = BetelgeuseAkkaClusteringPubSubExtension(system).send(ref, msg, pubSubMediator)
...
object BetelgeuseAkkaClusteringReceptionistExtension extends ExtensionId[BetelgeuseAkkaClusteringReceptionistExtension] with ExtensionIdProvider {

  override def get(system: ActorSystem): BetelgeuseAkkaClusteringReceptionistExtension = system.extension(this)

  override def apply(system: ActorSystem): BetelgeuseAkkaClusteringReceptionistExtension = system.extension(this)

  override def lookup(): BetelgeuseAkkaClusteringReceptionistExtension.type = BetelgeuseAkkaClusteringReceptionistExtension

  override def createExtension(system: ExtendedActorSystem): BetelgeuseAkkaClusteringReceptionistExtension =
    new BetelgeuseAkkaClusteringReceptionistExtension(system)
